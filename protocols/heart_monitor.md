...
Garden snails (*Helix aspersa*) are fairly large terrestrial invertebrates (more than 10g for adults)
...
As opposed to a closed circulatory system (e.g. vertebrates), the hemolymph then mixes with the extracellular fluid before diffusing to the lung.

The heart rates of snails is regulated by oxygen demand and can scale from more to 1 beat per second (e.g. when the animal is active and the temperature high) to as low as one a minute (e.g. during hibernation).
...
In human, pulseoximetry can be used to reliably and non-invasively infer heart rate by comparing red and infrared absorbance.
...
In this project, we will power an LED with a forward current `I_F = 40mA`) and a forward voltage `V_F = 2.6V` from a `V_B = 5V` board. 
Therefore, we need to drop `V_R = V_B - V_F = 2.4V` with a resistor (in series with the LED).

**@edwbaker => maths formatting around here?**

Therefore the resistance is `R = V_R/I_V`. So, in our case, `R = 60Ω`, which means we should be able to use a 68Ω resistor.

Phototransistors (PTs) are essentially transistors that allow variable current to pass through according to light intensity.
...
The value of the resistance affect the gain and should depend on the type of phototransistor and the light intensity. 
In our case, a 100kΩ resistor should give a satisfying range of results.  
...
2. Cut two short wires (1cm). Strip and twist their tips.
...
For this reason, a 4min video demonstrating this part of the protocol step by step is available online (see 'Supporting Material' section above).
...
6. Move the LED on the shell until you see the beating heart.
7. Use blu tac to maintain the circuit on the shell, and assess the quality of the signal displayed on your screen.
8. When the signal is satisfying (sharp oscillations of large amplitude), glue PT and LED to the shell.
9. Unplug the circuit, interrupt the `python` program, and wait for the glue to dry.
1. Plug the circuit. This time, **use the analogue pin** as opposed to the 5v.
...
Therefore, the actual reading sampling frequency `Fs_a = Fs * s` where `Fs` is the resulting sampling frequency (i.e. 5Hz) and `s`, the oversampling factor (i.e. 16 times).
The delay `dt` between two actual reads is then simply `1/Fs_a` (in seconds).
Thus, the number of milliseconds to sleep between reads is:

```C++
const float time_to_sleep_ms = 1e3/(FS*OVER_SAMPLING) - RISE_TIME;
```
Note that we also subtract the rise time to be more accurate. and that we multiply by 1000 since we want milliseconds instead of seconds.
...
2. We are going to sample as many time as the value of `OVER_SAMPLING`, so we start a for loop in which we:
    ...
    1. read the phototransistor value.
...
The corresponding power spectrum indicates a fundamental frequency around 0.75Hz (vertical dashed line), which is 45 beats per minute:
